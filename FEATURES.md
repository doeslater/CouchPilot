...
| Loads providers + trending shows concurrently at init; each collection is defined instantly with `shows = null` and only actually queried once its row scrolls into view (`loadCollectionShows()`), so opening Discover doesn't fire a dozen genre/network-discover calls up front. Re-fetches trending on provider filter change, preserving any collections (including ones hydrated concurrently mid-filter-change) and job-cancelling rapid taps. Correctly handles/resets hydration tracking if the Success state is lost during a failed filter change. | `discover/presentation/DiscoverViewModel.kt` |
